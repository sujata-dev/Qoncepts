...
my ($testno, $sem, $subject, $marks, $maxmarks, $flag);
...
sub body_section
{
    ...
    {
        $testno = $row;
        my $sth1 = $dbh -> prepare("select Semester, Subject,
                                    sum(AllotedMarks), MaxMarks
                                    from QuizAnswersDB natural join QuizDB
                                    where AllotedMarks in
                                    (select AllotedMarks
                                    from QuizAnswersDB
                                    where PRN=? and TestNo=?)");
        $sth1 -> execute($prn, $testno) or die;
        while (($sem, $subject, $marks, $maxmarks) =
                                            $sth1 -> fetchrow_array())
        {
            print "
                <b>
                ...
                Semester: $sem<br>
                Subject: $subject<br>
                Marks: $marks/$maxmarks<br><br><br></b>";
