...
sub body_section
{
    ...
    for (1..$noq)
    {
        print "
            ...
            <input id='marks$_' type='text' class='form-control' name='marks$_' placeholder='Marks' style='width:70px;'> <br>
            <textarea name='question$_' rows='6' cols='60' placeholder='Enter your question here'></textarea>
            <br><br><br>
            </b></h4>";
    }
    print "
...
sub info_insertion
{
    my $sth = $dbh -> prepare("insert into QuizDB values(?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $sth -> execute($testno, $branch, $year, $batch, $semester, $subject, $maxmarks, $noq, $subdate) or die;
    $sth -> finish();
}
